...
def create_model(num_classes, device):
    ...
    dh_model = torch.load('AOD_net_epoch_relu_10.pth', map_location=device)

    return od_model, dh_model
...
def main(parser_data):
    ...
    od_model, dh_model = create_model(num_classes=6, device=device)

    od_model.to(device)
    dh_model.to(device)

    params_od = [p for p in od_model.parameters() if p.requires_grad]
    optimizer_od = torch.optim.SGD(
        params_od, lr=0.005, momentum=0.9, weight_decay=0.0005
    )

    lr_scheduler_od = torch.optim.lr_scheduler.StepLR(
        optimizer_od, step_size=5, gamma=0.33
    )

    # 如果指定了上次训练保存的权重文件地址，则接着上次结果接着训练
    if parser_data.resume != "":
        checkpoint = torch.load(parser_data.resume, map_location=device)
        od_model.load_state_dict(checkpoint['od_model'])
        optimizer_od.load_state_dict(checkpoint['optimizer_od'])
        lr_scheduler_od.load_state_dict(checkpoint['lr_scheduler_od'])
    ...
    for epoch in range(parser_data.epochs):
        utils.train_one_epoch(od_model, dh_model, optimizer_od, train_dataloader, device, epoch, train_loss=train_loss
                              , train_lr=learning_rate, print_freq=50, warmup=True)

        lr_scheduler_od.step()

        utils.evaluate(od_model, dh_model, val_dataloader, device=device, mAP_list=val_mAP)

        # save weights
        save_files = {
            'od_model': od_model.state_dict(),
            'optimizer_od': optimizer_od.state_dict(),
            'lr_scheduler_od': lr_scheduler_od.state_dict(),
            'epoch': epoch}
