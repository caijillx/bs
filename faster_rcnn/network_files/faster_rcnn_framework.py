...
from faster_rcnn.network_files.roi_head import RoIHeads
from faster_rcnn.network_files.transform import GeneralizedRCNNTransform
from faster_rcnn.network_files.rpn_function import AnchorsGenerator, RPNHead, RegionProposalNetwork
...
class FasterRCNNBase(nn.Module):
    ...
    def __init__(self, backbone, rpn, roi_heads, transform):
        ...
        ## backbone为resnet50去掉最后的average pooling+fc-softmax层+fpn结构，其中fpn一共有五层，分别为layer1，layer2，layer3，layer4，layer4+maxpooling，其outchannel为256
        self.rpn = rpn
...
class FasterRCNN(FasterRCNNBase):
    ...
    Arguments:
        ...
        image_mean: mean values used for input normalization.
            They are generally the mean values of the dataset on which the backbone has been trained
            on
        image_std: std values used for input normalization.
    ...
    def __init__(self, backbone, num_classes=None,
        ...
        ### image_mean = [0.485, 0.456, 0.406],image_std = [0.229, 0.224, 0.225]为ImageNet的均值和方差数据
        ### 经过去雾处理的的数据为([0.38417724, 0.37856016, 0.3720098], [0.19573975, 0.21080585, 0.2189724])
        ### RTTS本身([0.55139, 0.5484414, 0.5413616], [0.19298169, 0.19455253, 0.19830151])

        if image_mean is None:
            # image_mean = [0.485, 0.456, 0.406]
            # image_mean = [0.384, 0.378, 0.372]
            image_mean = [0.551, 0.548, 0.541, 0.384, 0.378, 0.372]
        if image_std is None:
            # image_std = [0.229, 0.224, 0.225]
            # image_std = [0.195, 0.210, 0.219]
